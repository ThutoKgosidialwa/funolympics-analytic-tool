...
if geoip_file is not None and uploaded_file is not None:
    ...
    if not filtered_df.empty:
        ...
        chart1 = alt.Chart(chart_data).mark_bar().encode(
            x=alt.X('Sport:N', title='Sport', sort='-y', axis=alt.Axis(labelAngle=-45)),
    ...
    chart2 = alt.Chart(hourly_visits_df).mark_line(point=True).encode(
    ...
    chart3 = alt.Chart(filtered_df).mark_boxplot(extent='min-max').encode(
        x='sport:N',
        y='time_spent:Q',
        color='sport:N',
